...
type DeviceNodeData = Node<{
  label: string;
  deviceType: "Input" | "Output" | "Application";
}>;

const sliderStyle = { width: '100%', accentColor: '#00e5ff', marginTop: '10px', cursor: 'pointer' };

const nodeStyle = {
  background: '#1e1e1e', border: '1px solid #333', borderRadius: '8px',
  padding: '15px', minWidth: '200px', color: 'white',
  boxShadow: '0 4px 15px rgba(0,0,0,0.5)', textAlign: 'left' as const
};
...
export default function DeviceNode({ data, isConnectable }: NodeProps<DeviceNodeData>) {
  ...
  const isApp = data.deviceType === 'Application';
  
  let accentColor = '#22C55E';
  if (isInput) accentColor = '#2F8CFF';
  if (isApp) accentColor = '#A855F7';
  ...
  return (
    <div style={{ ...nodeStyle, borderTop: `4px solid ${accentColor}` }}>
      ...
      <button 
        ...
        style={{
          marginTop: '10px', padding: '5px', borderRadius: '4px', cursor: 'pointer',
          fontSize: '11px', fontWeight: 'bold', transition: '0.2s', border: 'none', color: 'white', width: '100%',
